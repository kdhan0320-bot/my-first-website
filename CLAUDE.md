...
## 폴더 구조
아래 폴더 중 일부는 아직 실제로 존재하지 않을 수 있습니다. 앞으로의 운영 기준으로
문서에 먼저 반영해두며, 실제 폴더 생성은 별도 승인을 받은 뒤 진행합니다.
...
`README.md`, `CLAUDE.md`, `.github/workflows/`, `.env`, `.env.local`,
Supabase 관련 설정, 프로젝트 썸네일·포트폴리오 이미지·문서 자료.
(`.git`, `.github`, `.env`, `.env.local`, `_private/`는 위 "절대 보호"
기준을 그대로 따르고, `projects/` 하위 배포 관련 파일은 아래
"projects/ — 배포 대상 규칙"을 그대로 따릅니다.)
...
## 절대 보호
`.git`, `.github`, `.env`, `.env.local`, `_private/` 전체는 항상 보호합니다.
그 외 `projects/` 하위 파일 보호 기준은 위 "projects/ — 배포 대상 규칙"을 따릅니다.

## 절대 금지 (승인 없이는 어떤 경우에도 하지 않음)
...
이 항목들에 대한 조사·진단·읽기는 자유롭게 하되, 실제 변경은 매번 명시적인
사용자 승인을 받은 뒤에만 진행합니다.
...
- GitHub push, commit, PR 생성, merge는 사용자가 직접 승인하기 전까지 하지 않습니다.
...
- AI는 결과물을 대신 만든 도구가 아니라 보조 도구로 표현합니다(문구 예시는
  각 프로젝트 CLAUDE.md/README를 따릅니다).
