...
const EvidenceBadges = ({ project }) => {
  ...
  return (
    <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.75, mb: 2 }}>
      {badges.map((label) => {
        /* Signal Orange는 완료 의미에만 제한적으로 사용 — 나머지 배지는 중립 톤 유지 */
        ...
        return (
          <Box
            ...
            sx={{
              px: 1.1, py: 0.35,
              borderRadius: 999,
              bgcolor: isComplete ? 'rgba(255,107,61,0.08)' : 'rgba(184,193,203,0.1)',
              border: isComplete ? `1px solid rgba(255,107,61,0.32)` : '1px solid rgba(184,193,203,0.24)',
            }}
          >
            <Typography sx={{ fontSize: '0.875rem', fontWeight: 700, color: isComplete ? COLORS.signalOrange : 'text.secondary', letterSpacing: '0.02em' }}>
              {label}
            </Typography>
          </Box>
        );
